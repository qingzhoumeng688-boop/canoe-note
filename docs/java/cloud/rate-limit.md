...
flowchart TD
    subgraph NORMAL["正常情况"]
        A1["服务A"] --> B1["服务B (50ms)"] --> C1["服务C"]
    end
    subgraph AVALANCHE["雪崩过程"]
        C2["服务C 变慢 (2s)"] --> B2["服务B 的线程卡在等待 C，线程池耗尽"]
        B2 --> A2["服务A 的线程卡在等待 B，线程池耗尽"]
        A2 --> DOWN["整条链路挂掉，连不依赖 C 的接口也挂了"]
    end
```
