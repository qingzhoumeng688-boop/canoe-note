...
```text
            XADD 追加消息（ID 单调递增）
                    ...
                    ▼
        ┌──────────────────────────┐
        │      order-stream (流)     │  消息: 1001, 1002, 1003, 1004 ...
        └──────────────────────────┘
                    │
        ┌───────────┴────────────┐
        │   消费组 group-1         │  XREADGROUP 时按负载均衡分给组内消费者
        │  ┌────────┐ ┌────────┐  │
        │  │ 消费者  │ │ 消费者  │  │  每个消息只给组内一个消费者（竞争消费）
        │  │  c-1   │ │  c-2   │  │
        │  └────────┘ └────────┘  │
        └──────────────────────────┘
                    │
        PEL（Pending Entries List）：记录"已投递但还没 XACK"的消息
        ┌──────────────────────────────────────┐
        │ 1003 → c-1（空闲 30s，投递 1 次）       │  XCLAIM 可把它转给 c-2 重试
        └──────────────────────────────────────┘
```
