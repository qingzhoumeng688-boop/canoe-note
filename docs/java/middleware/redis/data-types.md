...
│
├─ 要按"分数/权重"排序或排名吗？
│   └─ 是 → 用 ZSet（有序集合）           例：排行榜、延迟队列、热搜
│
├─ 要去重吗（同一元素不能重复出现）？
│   └─ 是 → 用 Set（集合）                例：兴趣标签、共同好友、抽奖
│
├─ 要存"一个对象的多个字段"且常单独改某字段吗？
│   └─ 是 → 用 Hash（哈希）              例：购物车、用户资料
│
├─ 要"先进先出 / 后进先出 / 时间线"吗？
│   └─ 是 → 用 List（列表）             例：消息队列、最新动态
│
└─ 只是简单的"一个 key 一个值"、计数、带过期？
    └─ 是 → 用 String（字符串）         例：缓存、验证码、计数器、Session
```
