...
```mermaid
flowchart TD
    NS["NameServer 集群（无状态）：类比 114 查号台 / DNS 服务器，节点之间互相不通信，各存一份路由表"]
    BA["Broker-A（Master）"] -->|"注册路由：每 30s 心跳"| NS
    BB["Broker-B（Slave）"] -->|"注册路由：每 30s 心跳"| NS
    NS -->|"拉取路由"| P["Producer（生产者）"]
    NS -->|"拉取路由"| C["Consumer（消费者）"]
    P -->|"发消息"| BC["Broker 集群：真正存消息、收发消息的地方"]
    BC -->|"拉消息"| C
    BC --> BA
    BC --> BB
    BB -->|"主从同步"| BA
...
```mermaid
flowchart TD
    T["Topic: order_topic"] --> Q0["Queue-0（Broker-A）"]
    T --> Q1["Queue-1（Broker-A）"]
    T --> Q2["Queue-2（Broker-B）"]
    T --> Q3["Queue-3（Broker-B）"]
    Q0 --> C1["Consumer-1（负责 Q0、Q1）"]
    Q1 --> C1
    Q2 --> C2["Consumer-2（负责 Q2、Q3）"]
    Q3 --> C2
    C1 --> N["每条消息只被消费一次"]
    C2 --> N
```
...
flowchart TD
    ...
    T --> Q2["Queue-2（Broker-B）"]
    T --> Q3["Queue-3（Broker-B）"]
    Q0 --> C1["Consumer-1（消费全部 4 个队列）"]
    Q1 --> C1
    Q2 --> C1
    Q3 --> C1
    Q0 --> C2["Consumer-2（也消费全部 4 个队列）"]
    Q1 --> C2
    Q2 --> C2
    Q3 --> C2
    C1 --> N["每条消息被每个消费者各消费一次"]
    C2 --> N
```
