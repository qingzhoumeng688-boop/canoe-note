...
classDiagram
    ...
    class GoPieceFactory {
        -Map pool
        +getPiece(String color) GoPiece
        +getPoolSize() int
    }
    class GoPiece {
        +draw(int x, int y)
    }
    class ConcreteGoPiece {
        -String color
        +draw(int x, int y)
    }
    Client --> GoPieceFactory : 传入内部状态 color 取棋子
    GoPieceFactory o-- GoPiece : 缓存池，同色唯一
    GoPiece <|.. ConcreteGoPiece : 具体享元
    Client ..> GoPiece : 传入外部状态 draw(x, y)
```
...
| 对比维度 | 内部状态 InternalState | 外部状态 ExternalState |
| --- | --- | --- |
| 存放位置 | 存在享元对象内部 | 由客户端持有、调用时传入 |
| 与场景关系 | 与具体场景无关 | 随使用场景变化 |
| 可否共享 | 可安全共享（不变） | 不可共享（会变） |
| 例子 | 棋子颜色 = 黑/白 | 棋子坐标 = (x, y) |
