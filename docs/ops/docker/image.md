...
```mermaid
flowchart TD
    CL["容器层 Container Layer（可读写）"] --> L4["镜像层 N：COPY app.jar /app/"]
    L4 --> L3["镜像层 3：RUN apt-get install -y curl"]
    L3 --> L2["镜像层 2：RUN apt-get update"]
    L2 --> L1["镜像层 1（基础层）：FROM ubuntu:22.04"]
    L1 --> FS["通过 UnionFS 联合挂载，对外呈现为一个完整的文件系统"]
```
...
```mermaid
flowchart TD
    R1["读取：容器要读 /etc/hosts"] --> R2["自上而下查找：容器层没有 → 镜像层N没有 → 在基础层找到"] --> R3["直接读取该文件的下层版本"]
    M1["修改：容器要改基础层的 /etc/nginx/nginx.conf"] --> M2["Docker 不会去修改只读的镜像层"] --> M3["把该文件复制到容器可写层"] --> M4["修改容器层里的这份副本"] --> M5["上层覆盖下层，以后读到的都是新版本"]
    D1["删除：容器要删基础层的 /tmp/foo"] --> D2["也不会真去删镜像层"] --> D3["在容器层创建 whiteout 白障文件遮挡下层"] --> D4["对外看起来就是删掉了"]
```
